...
export class SubscriptionHandshakeLink extends ApolloLink {
  ...
  private topicObservers: Map<
    string,
    Set<ZenObservable.Observer<any>>
  > = new Map();
  ...
  request(operation: Operation) {
    ...
    const {
      extensions: { subscription: { newSubscriptions, mqttConnections } } = {
    ...
    const newSubscriptionTopics = Object.keys(newSubscriptions).map(
    ...
    return new Observable<FetchResult>((observer) => {
      existingTopicsWithObserver.forEach((t) => {
        ...
        const anObserver = Array.from(this.topicObservers.get(t)).find(
          () => true
        );

        const [clientId] = Array.from(
          this.clientObservers
        ).find(([, { observers }]) => observers.has(anObserver));
      ...
      return () => {
        const clientsForCurrentObserver = Array.from(
          this.clientObservers
        ).filter(([, { observers }]) => observers.has(observer));
        clientsForCurrentObserver.forEach(([clientId]) =>
          this.clientObservers.get(clientId).observers.delete(observer)
        );
        ...
        this.clientObservers = new Map(
          Array.from(this.clientObservers).filter(
            ([, { observers }]) => observers.size > 0
          )
        );
  ...
  async connectNewClients(
    ...
    const selectionNames = (getMainDefinition(query).selectionSet
      .selections as FieldNode[]).map(({ name: { value } }) => value);

    const result = Promise.all(
      connectionInfo.map((c) =>
        this.connectNewClient(c, observer, selectionNames)
      )
    );
  ...
  async connectNewClient(
        ...
        topics.forEach((t) => {
          if (this.topicObservers.has(t)) {
            this.topicObservers
              .get(t)
              .forEach((observer) =>
                observer.error({ ...args, permanent: true })
              );
          }
        });
      }
    ...
    (client as any).onMessageArrived = ({ destinationName, payloadString }) =>
      this.onMessage(destinationName, payloadString, selectionNames);
  ...
  subscribeToTopics<T>(
    ...
    return Promise.all(
      topics.map((topic) => this.subscribeToTopic(client, topic, observer))
    );
  }
      ...
      (client as any).subscribe(topic, {
        onSuccess: () => {
          ...
          if (!this.clientObservers.has(client.clientId)) {
            this.clientObservers.set(client.clientId, {
              client,
              observers: new Set(),
            });
          }
