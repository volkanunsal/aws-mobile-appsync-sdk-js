import { AUTH_TYPE } from '@volkanunsal/aws-appsync-auth-link';
...
describe('RealTime subscription link', () => {
  ...
  test('Initialize WebSocket correctly for API KEY', (done) => {
    ...
    AppSyncRealTimeSubscriptionHandshakeLink.createWebSocket = jest.fn(
      (url, protocol) => {
        expect(url).toBe(
          'wss://xxxxx.appsync-realtime-api.amazonaws.com/graphql?header=eyJob3N0IjoieHh4eHguYXBwc3luYy1hcGkuYW1hem9uYXdzLmNvbSIsIngtYW16LWRhdGUiOiIyMDE5MTExM1QxODQ3MDRaIiwieC1hcGkta2V5IjoieHh4eHgifQ==&payload=e30='
        );
  ...
  test('Initialize WebSocket correctly for COGNITO USER POOLS', (done) => {
    ...
    AppSyncRealTimeSubscriptionHandshakeLink.createWebSocket = jest.fn(
      (url, protocol) => {
        expect(url).toBe(
          'wss://xxxxx.appsync-realtime-api.amazonaws.com/graphql?header=eyJBdXRob3JpemF0aW9uIjoidG9rZW4iLCJob3N0IjoieHh4eHguYXBwc3luYy1hcGkuYW1hem9uYXdzLmNvbSJ9&payload=e30='
        );
        expect(protocol).toBe('graphql-ws');
        done();
        return new myWebSocket();
