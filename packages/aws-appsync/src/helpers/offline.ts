...
export const getOpTypeFromOperationName = (
  opName = ''
): CacheOperationTypes => {
  // Note: we do a toLowerCase() and startsWith() to avoid ambiguity with operations like "RemoveAddendum"
  const comparator = (prefix) =>
    opName.toLowerCase().startsWith(prefix) ||
    opName.toLowerCase().startsWith(`on${prefix}`);
...
export type CacheUpdatesOptions =
  | ((variables?: object) => CacheUpdatesDefinitions)
  | CacheUpdatesDefinitions;
...
const buildSubscription = (
  ...
  const document =
    (subscriptionQuery && (subscriptionQuery as QueryWithVariables).query) ||
    (subscriptionQuery as DocumentNode);
  const variables =
    (subscriptionQuery &&
      (subscriptionQuery as QueryWithVariables).variables) ||
    ({} as OperationVariables);

  const query =
    (cacheUpdateQuery && (cacheUpdateQuery as QueryWithVariables).query) ||
...
const getOpTypeQueriesMap = (
  cacheUpdateQuery: CacheUpdatesOptions,
  variables
): CacheUpdatesDefinitions => {
  const cacheUpdateQueryVal =
    typeof cacheUpdateQuery === 'function'
  ...
  if (
    isDocument(cacheUpdateQueryVal) ||
    isDocument((cacheUpdateQueryVal as QueryWithVariables).query) ||
    Array.isArray(cacheUpdateQuery)
  ) {
    opTypeQueriesMap = {
      [CacheOperationTypes.AUTO]: [].concat(cacheUpdateQueryVal),
    } as CacheUpdatesDefinitions;
  }

  return opTypeQueriesMap;
};

const getEvaluatedOp = (
  opType: CacheOperationTypes,
  mutationField: string,
  operationType: CacheOperationTypes
) => {
  const evaluatedOP =
    opType === CacheOperationTypes.AUTO
...
const setValueByPath = <T>(obj: T, path: string[] = [], value): T =>
...
const buildMutation = <T = OperationVariables>(
  ...
  const isVariablesInfo =
    typeof (variablesInfo as VariablesInfo).variables === 'object';
  const variables = isVariablesInfo
    ? (variablesInfo as VariablesInfo).variables
    : (variablesInfo as T);

  const hasInputType =
    Object.keys(variables).length === 1 && typeof variables.input === 'object';

  const inputTypeVersionField =
    isVariablesInfo &&
    ((variablesInfo as VariablesInfo).inputType
      .definitions[0] as InputObjectTypeDefinitionNode).fields.find(
      (f) =>
        ['version', 'expectedVersion'].find((n) => n === f.name.value) &&
        (f.type as NamedTypeNode).name.value === 'Int'
    );
  ...
  const { [idField || 'id']: idCustomField } = hasInputType
    ? variables.input
    : variables;
  ...
  for (let opType in opTypeQueriesMap) {
    ...
    queries.forEach((queryEntry) => {
      const query =
        (queryEntry && (queryEntry as QueryWithVariables).query) ||
        (queryEntry as DocumentNode);
      const queryVars =
      ...
      let cachedItem;
      try {
        const { [queryField]: queryRead } = client.readQuery<{
          [key: string]: any;
  ...
  const cache: { getIdsMap: () => object } =
    client &&
    client instanceof AWSAppSyncClient &&
    (client as AWSAppSyncClient<any>).isOfflineEnabled() &&
    (client.cache as any);

  const versionFieldName: string = inputTypeVersionField
    ? inputTypeVersionField.name.value
    : '';
  ...
  return {
    ...
    variables: hasInputType
      ? {
          input: {
            ...(useVersioning && { [versionFieldName]: version }),
            ...variables.input,
          },
        }
    ...
    optimisticResponse: typename
      ? {
          ...
          [mutationField]: {
            __typename: typename,
            [idField]:
              (hasInputType ? variables.input : variables)[idField] || uuid(),
    ...
    update: (proxy, { data: { [mutationField]: mutatedItem } }) => {
      for (let opType in opTypeQueriesMap) {
        ...
        const updaterFn = getUpdater(
          getEvaluatedOp(
            opType as CacheOperationTypes,
            mutationField,
            operationType
          ),
          idField
