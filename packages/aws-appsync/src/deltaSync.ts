...
type SubscribeWithSyncEffectOptions<
  T,
  TVariables = OperationVariables
> = SubscribeWithSyncOptions<T, TVariables> & {
  lastSyncTimestamp?: number;
  baseLastSyncTimestamp?: number;
};
...
type SubscriptionMessagesProcessorCreator = (
  proxy: DataProxy,
  updateFunction: MutationUpdaterFn
) => SubscriptionMessagesProcessor;
...
type DeltaSyncReducer = () => (
  state: AppSyncMetadataState,
  action: AnyAction
) => AppSyncMetadataState;
//#endregion
...
//#region helpers
const subscriptionMessagesProcessorCreator: SubscriptionMessagesProcessorCreator = (
  proxy,
  updateFunction
) => {
  let buffer = [];
  ...
  const wrappedUpdateFunction: MutationUpdaterFn = (proxy, record) =>
    tryFunctionOrLogError(() => updateFunction(proxy, record));
...
const effect = async <TCache extends NormalizedCacheObject>(
  ...
  const {
    options,
    options: { baseQuery, subscriptionQuery, deltaQuery },
    observer,
    callback = () => {},
  } = effect;
  ...
  const itemInHash = store.getState()[METADATA_KEY][DELTASYNC_KEY].metadata[
    hash
  ];
  let {
  ...
  const enqueueAgain = () => {
    ...
    boundEnqueueDeltaSync(
      store,
      { ...options, lastSyncTimestamp, baseLastSyncTimestamp },
      observer,
      callback
    );
  };
  ...
  if (typeof callback === 'function') {
    let handle = new Observable(() => () => unsubscribeAll()).subscribe({
      next: () => {},
    });
    callback(handle);
  }
  ...
  const STOP_CACHE_RECORDING =
    typeof Symbol !== 'undefined'
      ? Symbol('stopCacheRecording')
      : '@@stopCacheRecording';
  ...
  subscriptionProcessor = subscriptionMessagesProcessorCreator(
    cacheProxy,
    (proxy, record) => {
  ...
  try {
      ...
      if (subscriptionQuery && subscriptionQuery.query) {
        ...
        subscription = client
          ...
          .filter((data) => {
            const {
              extensions: {
                controlMsgType = undefined,
                controlMsgInfo = undefined,
              } = {},
            } = data;
    ...
    upperBoundTimeMS = baseRefreshIntervalInSeconds
      ? baseRefreshIntervalInSeconds * 1000
      : DEFAULT_UPPER_BOUND_TIME_MS;

    const skipBaseQuery =
      !(baseQuery && baseQuery.query) ||
      (baseLastSyncTimestamp
        ? Date.now() - baseLastSyncTimestamp < upperBoundTimeMS
        : itemInHash.baseLastSyncTimestamp &&
          Date.now() - itemInHash.baseLastSyncTimestamp < upperBoundTimeMS);
    ...
    if (deltaQuery && deltaQuery.query && skipBaseQuery) {
      ...
      const result = await client.query({
        ...
        variables: {
          ...variables,
          lastSync:
            Math.floor((lastSyncTimestamp || baseLastSyncTimestamp) / 1000) ||
            0,
        },
      });
    ...
    if (baseQuery && baseQuery.query) {
      ...
      baseQueryTimeoutId = (global as any).setTimeout(
        () => enqueueAgain(),
        baseQueryTimeout
      );
    }
...
const reducer: DeltaSyncReducer = () => (
  state: AppSyncMetadataState,
  action: AnyAction
) => {
  switch (action.type) {
    ...
    case actions.ENQUEUE:
      logger(
        action.type,
        ((action as OfflineAction).meta.offline.effect as any).options
      );
...
const lastSyncReducer = (
  state: AppSyncMetadataState,
  action: DeltaSyncUpdateLastSyncAction
) => {
  const {
    payload: { lastSyncTimestamp, hash, baseLastSyncTimestamp },
  } = action;
  ...
  const newMetadata: DeltaSyncStateMetadata = {
    baseLastSyncTimestamp:
      baseLastSyncTimestamp || hashMetadata.baseLastSyncTimestamp,
    lastSyncTimestamp,
  };
...
const metadataReducer = (
  ...
  const newMetadata: DeltaSyncStateMetadata = {
    lastSyncTimestamp,
    baseLastSyncTimestamp:
      options.baseLastSyncTimestamp === null ? null : baseLastSyncTimestamp,
  };
...
const writeCacheUpdates = <T = { [key: string]: any }>(
  ...
  result.forEach((item) =>
    cacheUpdates(item).forEach(({ query, variables }) => {
...
>(
  ...
  const {
    name: { value: opName },
    alias: opAliasNode,
  } = opDefinition.selectionSet.selections[0] as FieldNode;
  const { value: opAlias = null } = opAliasNode || {};

  const {
    kind,
    operation: graphqlOperation,
  } = opDefinition as OperationDefinitionNode;
  const isSubscription =
    kind === 'OperationDefinition' && graphqlOperation === 'subscription';

  const [deltaOperationName] = isSubscription
    ? Object.keys(data)
    : [opAlias || opName];
  ...
  if (!baseQuery || !baseQuery.query) {
    ...
    const { [operationName]: baseResult } = cache.readQuery<{
      [key: string]: any;
    }>({ query, variables });
    ...
    const result = deltaRecordsProcessor<T>(
      updateLogger,
      deltaOperationName,
      deltaRecords,
      baseResult,
...
export type BuildBaseQuerySyncOptions<
  T,
  TVariables = OperationVariables
