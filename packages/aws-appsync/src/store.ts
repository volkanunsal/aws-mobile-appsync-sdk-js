    ...
    compose(
      ...
      offline({
        ...
        discard: (error, action, retries) =>
          discard(callback, error, action, retries),
      })
    )
  );
...
export declare type OfflineStatusChangeCallbackCreator = (
...
const offlineEffectsConfigs = [mutationsConfig, deltaSyncConfig]
  .filter(Boolean)
  .reduce((acc, { enqueueAction, ...rest }) => {
    acc[enqueueAction] = { enqueueAction, ...rest };

    return acc;
  }, {}) as OfflineEffectConfigMap;

const reducer: (dataIdFromObject: IdGetter) => ReducersMapObject = <S>(
  dataIdFromObject
) => ({
  [METADATA_KEY]: (state: S, action: AnyAction) =>
    Object.entries(offlineEffectsConfigs).reduce(
      (acc, [, { reducer = () => (x) => x }]) =>
        reducer(dataIdFromObject)(acc, action),
      state
    ),
});
...
const effect = async (
  ...
  clientGetter,
  callback,
  offlineStatusChangeCallbackCreator: OfflineStatusChangeCallbackCreator
) => {
