...
const getOutbox = <T extends NormalizedCacheObject>(
  client: AWSAppSyncClient<T>
) => getStoreState(client).offline.outbox;
...
describe('Offline disabled', () => {
  ...
  test('Conflict resolution (offline disabled)', async () => {
        ...
        errors: [
          {
            ...backendError,
            data: {
              ...serverResponse.addTodo,
              version: serverResponse.addTodo.version + 1,
            },
          },
        ],
      },
        ...
        errors: [
          {
            ...
            data: {
              ...serverResponse.addTodo,
              version: serverResponse.addTodo.version + 2,
            },
          },
        ],
      },
    ...
    const resultPromise = client.mutate({
      mutation: gql`
        mutation($name: String!, $expectedVersion: Integer) {
          addTodo(name: $name, expectedVersion: $expectedVersion) {
            id
            name
            version
          }
...
describe('Offline enabled', () => {
  ...
  test('conflict resolution', async () => {
    ...
    const conflictResolver: ConflictResolver = jest.fn(
      ({ variables, data, ...rest }) => {
        return {
          ...variables,
    ...
    const resultPromise = client.mutate({
      mutation: gql`
        mutation($name: String!, $version: Integer) {
          addTodo(name: $name, expectedVersion: $version) {
            id
            name
            version
          }
    ...
    try {
      ...
      expect(callback).toHaveBeenCalledWith(null, {
        ...
        variables: {
          ...variables,
          expectedVersion: variables.expectedVersion + 1,
        },
        mutation: 'addTodo',
        notified: true,
      });
    ...
    const child = await client.mutate({
      mutation: gql`
        mutation($parentId: ID, $name: String!) {
          addChild(parentId: $parentId, name: $name) {
            id
            parentId
            name
          }
...
describe('Multi client', () => {
  ...
  test.each([false, null, ''])(
    'Uses default prefix for falsey (%o) keyPrefix',
    async (keyPrefix: any) => {
      const storage = new MemoryStorage();
      mockHttpResponse({
        data: {
      ...
      expect(allKeys.length).toBeGreaterThan(0);
      allKeys.forEach((key) =>
        expect(key).toMatch(new RegExp(`^${DEFAULT_KEY_PREFIX}:.+`))
      );
    }
  );
  ...
  test('Can use different prefixes', async () => {
    ...
    for (let keyPrefix of prefixes) {
      ...
      expect(allKeys.length).toBeGreaterThan(0);
      allKeys.forEach((key) =>
        expect(key).toMatch(new RegExp(`^${keyPrefix}:.+`))
      );
    }
  ...
  test('Cannot use same keyPrefix more than once', () => {
    ...
    expect(() => {
      ...
      'The keyPrefix myPrefix is already in use. Multiple clients cannot share the same keyPrefix.'
    );
  });
});
